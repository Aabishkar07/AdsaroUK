...
export default function LocationTargeting({
  ...
  useEffect(() => {
    if (propSelectedIds !== undefined) {
      ...
      console.log(
        "🔄 Location Component: Edit mode - using provided selectedIds:",
  ...
  const buildLocationTree = useCallback(
    async (
      range: { start: number; end: number } = { start: 0, end: 500 },
      ...
      try {
        const [countryRes, regionRes, cityRes, postalRes] = await Promise.all([
          axios.get<ApiResponse>(
            `https://panel.adsaro.com/advertiser/api/GeoCountries/?version=4&token=${auth.token}&range=${range.start}-${range.end}`
          ),
          axios.get<ApiResponse>(
            `https://panel.adsaro.com/advertiser/api/GeoRegions/?version=4&token=${auth.token}&range=${range.start}-${range.end}`
          ),
          axios.get<ApiResponse>(
            `https://panel.adsaro.com/advertiser/api/GeoCities/?version=4&token=${auth.token}&range=${range.start}-${range.end}`
          ),
          axios.get<ApiResponse>(
            `https://panel.adsaro.com/advertiser/api/GeoPostalCodes/?version=4&token=${auth.token}&range=${range.start}-${range.end}`
          ),
        ]);

        const countries = Object.values(
          countryRes.data?.response?.rows || {}
        ) as CountryData[];
        const regions = Object.values(
          regionRes.data?.response?.rows || {}
        ) as RegionData[];
        const cities = Object.values(
          cityRes.data?.response?.rows || {}
        ) as CityData[];
        const postals = Object.values(
          postalRes.data?.response?.rows || {}
        ) as PostalData[];
        ...
        const countryGroups = new Map<
          string,
          { country: CountryData; regions: RegionData[] }
        >();

        countries.forEach((country) => {
          const countryRegions = regions.filter(
            (region) => region.country_iso === country.iso
          );
        ...
        countryGroups.forEach(({ country, regions: countryRegions }) => {
          ...
          countryRegions.forEach((region) => {
            ...
            regionCities.forEach((city) => {
              const cityPostals = postals.filter(
                (postal) => postal.city_id === city.id
              );
              ...
              newNodes.forEach((newNode) => {
                ...
                if (existingIndex >= 0) {
                  ...
                  if (newNode.children && result[existingIndex].children) {
                    result[existingIndex].children = mergeNodes(
  ...
  // Load all pages sequentially on mount
  const loadAllPages = useCallback(async () => {
    if (!auth.token) return;
    setLoading(true);
    setError("");
    setHasMoreData(true);
    // reset tree before full load
    setLocationTree([]);
    let start = 0;
    const PAGE = 1000;
    // Cap total iterations to avoid runaway in unexpected API behavior
    let safety = 0;
    while (safety < 200) {
  ...
  // Removed manual pagination; we now load all pages up front
  ...
  // Ensure search covers more pages progressively if no matches yet
  const ensureSearchCoverage = useCallback(
    async (query: string) => {
      if (!query.trim()) return;
      if (isSearching) return;
      try {
        setIsSearching(true);
        // Loop loading more until we either find matches or run out of data
        // Cap the number of additional iterations to avoid infinite loops
        let safety = 0;
        while (hasMoreData && safety < 50) {
          const currentFiltered = computeFiltered(locationTree, query);
          if (currentFiltered.length > 0) {
            // We have matches with the current tree
            break;
          }
          const start = currentRange.end + 1;
    ...
    [isSearching, hasMoreData, locationTree, currentRange, buildLocationTree, computeFiltered]
  );
  ...
  useEffect(() => {
    ...
    if (debouncedQuery.trim()) {
      ensureSearchCoverage(debouncedQuery);
    }
  }, [debouncedQuery, filterTree, ensureSearchCoverage]);
  ...
  const getLocationData = useCallback((): SelectedLocation[] => {
    ...
    const processSelectedNode = (node: LocationNode) => {
      if (selectedIds.has(node.id)) {
        ...
        if (node.type === "ZIP") {
          const cityNode = node.parentId
            ? findNodeById(locationTree, node.parentId)
            : null;
          const regionNode = cityNode?.parentId
            ? findNodeById(locationTree, cityNode.parentId)
            : null;
          const countryNode = regionNode?.parentId
            ? findNodeById(locationTree, regionNode.parentId)
            : null;
          ...
          const regionNode = node.parentId
            ? findNodeById(locationTree, node.parentId)
            : null;
          const countryNode = regionNode?.parentId
            ? findNodeById(locationTree, regionNode.parentId)
          ...
          const countryNode = node.parentId
            ? findNodeById(locationTree, node.parentId)
            : null;
  ...
  const handleSelectionChange = useCallback(
    (nodeId: string, checked: boolean) => {
      ...
      if (onDataChange) {
        ...
        console.log(
          "📍 Location component: Calling onDataChange with:",
          locationData
        );
  ...
  const isNodeIndeterminate = (node: LocationNode): boolean => {
    ...
    const someSelected = node.children.some(
      (child) => isNodeSelected(child) || isNodeIndeterminate(child)
    );
  ...
  useEffect(() => {
    if (onDataChange) {
      const locationData = getLocationData();
      console.log(
        "📍 Location component: useEffect calling onDataChange with:",
        locationData
      );
      onDataChange(locationData);
    }
    ...
    <div className="space-y-4">
      ...
      <div className="border border-gray-300 rounded-md w-full md:w-96 text-sm bg-white max-h-[450px] overflow-auto p-2 relative">
        {loading ? (
          ...
          <>
            ...
            {/* Full data is loaded; manual pagination removed */}
            ...
            {process.env.NODE_ENV === "development" && (
              <div className="mt-4 p-4 bg-gray-100 rounded-md text-xs">
                ...
                <div className="space-y-1">
                  ...
                  <div>
                    <strong>Selected Count:</strong> {selectedIds.size}
                  </div>
                  <div>
                    <strong>Location Data:</strong>{" "}
                    {JSON.stringify(getLocationData(), null, 2)}
                  </div>
                </div>
              </div>
            )}
      ...
      {selectedIds.size === 0 && filteredTree.length > 0 && !loading && (
        <div className="text-sm text-yellow-600 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
          <strong>Note:</strong> No locations selected. Your campaign may not
          receive traffic. Select at least one location to enable targeting.
        </div>
      )}
