...
// Define types for IFA list data
...
function IfaListDataTable() {
  ...
  const confirmDelete = async () => {
    if (selectedData) {
      try {
        const loadingToast = toast.loading("Deleting IFA List...");
        
        await axios.delete(
          `https://panel.adsaro.com/advertiser/api/IfaList/${selectedData.id}?version=4&token=${mytoken}`
        );
        
        toast.dismiss(loadingToast);
        toast.success(`IFA List "${selectedData.name}" deleted successfully!`);
        ...
        toast.error(`Failed to delete IFA List: ${message}`);
      }
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://panel.adsaro.com/advertiser/api/IfaList/?version=4&token=${mytoken}`
      );
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-700">IFA Lists</h1>
            ...
            <div className="bg-white py-3 shadow-md flex justify-between items-center rounded-lg p-6">
              <div className="">
                ...
                <div className="text-sm text-gray-500">
                  ...
                  <span className="text-gray-800 font-medium ml-1">IFA Lists</span>
                </div>
              </div>
            </div>
