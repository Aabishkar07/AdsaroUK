...
export default function Page() {
  return (
    <EditList 
      ...
      selectedData={{ id: 'id', name: 'name' }} 
      setShowEditModal={(value) => console.log(value)}
    />
  );
}
...
function EditList({ 
  ...
  useEffect(() => {
    if (selectedData) {
      ...
      if (selectedData.domains) {
        ...
        // Decode and display the existing domains
        console.log("Raw domains data:", selectedData.domains);
        console.log("Domains data type:", typeof selectedData.domains);
        console.log("Domains data length:", selectedData.domains.length);
        ...
        try {
          ...
          if (selectedData.domains.startsWith('data:')) {
            ...
            console.log("Data URL decoded result:", decodedDomains);
          } else {
            // Try direct base64 decoding
            decodedDomains = atob(selectedData.domains);
            console.log("Base64 decoded result:", decodedDomains);
          }
          
          console.log("Decoded length:", decodedDomains.length);
          
          // Check if the decoded result looks like valid text
          if (decodedDomains.includes('\n') || decodedDomains.includes('.') || decodedDomains.length > 0) {
            setFileContent(decodedDomains);
          } else {
            throw new Error("Decoded content doesn't look like domain data");
          }
        } catch (base64Error) {
          // If base64 decoding fails, try treating it as raw text
          console.log("Base64 decoding failed, trying as raw text:", base64Error);
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      // Check for success - be more flexible with success detection
      ...
      reader.onload = () => {
        try {
          ...
          if (invalidLines.length > 0) {
            const errorMessage = `Invalid domain names found: ${invalidLines.slice(0, 3).join(', ')}${invalidLines.length > 3 ? ` and ${invalidLines.length - 3} more` : ''}. Please ensure all lines contain valid domain names only.`;
            reject(new Error(errorMessage));
            return;
          }
          
          if (validDomains.length === 0) {
            reject(new Error('No valid domains found in the file. Please ensure the file contains valid domain names.'));
            return;
          }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div>
            ...
            <input 
              type="hidden" 
              {...register("domains", { 
                required: selectedData ? false : "Domain list file is required" 
              })} 
            />
            {errors.domains && (
              <p className="text-red-500 text-sm mt-1">
