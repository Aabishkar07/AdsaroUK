...
// Define types for app list data
...
function AppListDataTable() {
  ...
  const confirmDelete = async () => {
    if (selectedData) {
      try {
        const loadingToast = toast.loading("Deleting App List...");
        ...
        toast.success(`App List "${selectedData.name}" deleted successfully!`);
        ...
        toast.error(`Failed to delete App List: ${message}`);
      }
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        `https://panel.adsaro.com/advertiser/api/AppList/?version=4&token=${mytoken}`
      );
  ...
  return (
    <div className="w-full">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-700">App Lists</h1>
            ...
            <div className="bg-white py-3 shadow-md flex justify-between items-center rounded-lg p-6">
              <div className="">
                ...
                <div className="text-sm text-gray-500">
                  ...
                  <span className="text-gray-800 font-medium ml-1">App Lists</span>
                </div>
              </div>
            </div>
