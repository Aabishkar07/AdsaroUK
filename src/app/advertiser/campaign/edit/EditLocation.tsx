...
export default function EditLocationTargeting({ selectedIds: propSelectedIds, onSelectionChange, onDataChange }: LocationTargetingProps) {
  ...
  useEffect(() => {
    if (propSelectedIds !== undefined && propSelectedIds.size > 0) {
      // Edit mode: use the exact selectedIds provided
      ...
      // Auto-expand parent nodes for selected locations
      setTimeout(() => {
        const newExpandedIds = new Set<string>();
        const expandParents = (nodes: LocationNode[]) => {
          nodes.forEach(node => {
            if (propSelectedIds.has(node.id)) {
              // This node is selected, expand all parents
              let currentParent = node.parentId;
              while (currentParent) {
                newExpandedIds.add(currentParent);
                const parentNode = findNodeById(locationTree, currentParent);
        ...
        expandParents(locationTree);
        if (newExpandedIds.size > 0) {
          setExpandedIds(prev => new Set([...prev, ...newExpandedIds]));
          console.log("🔄 Auto-expanded parent nodes for selected locations:", Array.from(newExpandedIds));
        }
      }, 100);
    } else if (propSelectedIds !== undefined) {
      // Edit mode with empty selection
      setSelectedIds(new Set());
      console.log("🔄 Location Component: Edit mode - no previously selected locations");
    } else {
  ...
  const buildLocationTree = useCallback(async (range: { start: number; end: number } = { start: 0, end: 500 }, append: boolean = false) => {
    ...
    try {
      // Countries and Regions should be fetched WITHOUT range to get ALL data
      // Cities and Postals use range for pagination
      const [countryRes, regionRes, cityRes, postalRes] = await Promise.all([
        axios.get<ApiResponse>(`https://panel.adsaro.com/advertiser/api/GeoCountries/?version=4&token=${auth.token}`),
        axios.get<ApiResponse>(`https://panel.adsaro.com/advertiser/api/GeoRegions/?version=4&token=${auth.token}`),
      ...
      console.log("📍 Edit Location: Loaded", countries.length, "countries,", regions.length, "regions,", cities.length, "cities,", postals.length, "postals");
      if (countries.length > 0 && range.start === 0) {
        console.log("📍 First few country ISOs:", countries.slice(0, 5).map(c => `${c.iso} (${c.name})`).join(", "));
      }
      ...
      if (append) {
        ...
        setLocationTree(prevTree => {
          ...
          const merged = mergeNodes(prevTree, tree);
          console.log("📍 Edit Location: Tree merged - total countries now:", merged.filter(n => n.type === 'COUNTRY').length);
          return merged;
        });
      } else {
        setLocationTree(tree);
        console.log("📍 Edit Location: Initial tree loaded - total countries:", tree.filter(n => n.type === 'COUNTRY').length);
      }
  ...
  // Update selectedIds when location tree changes (for edit mode)
  useEffect(() => {
    if (propSelectedIds && propSelectedIds.size > 0 && locationTree.length > 0) {
      // Re-apply selectedIds after tree is loaded to ensure checkboxes are checked
      setSelectedIds(new Set(propSelectedIds));
      console.log("🔄 Location Component: Re-applied selectedIds after tree loaded:", Array.from(propSelectedIds));
      console.log("🔄 Location Component: Total locations to select:", propSelectedIds.size);
      
      // Expand parent nodes for selected locations
      const newExpandedIds = new Set<string>();
      const expandParents = (nodes: LocationNode[]) => {
        nodes.forEach(node => {
          if (propSelectedIds.has(node.id)) {
      ...
      if (newExpandedIds.size > 0) {
        setExpandedIds(prev => new Set([...prev, ...newExpandedIds]));
        console.log("🔄 Auto-expanded parent nodes:", Array.from(newExpandedIds));
      }
      
      // After setting selections, emit the location data to parent
      setTimeout(() => {
        const locationData = getLocationData();
        console.log("📍 Location Component: Emitting loaded location data to parent:", {
          count: locationData.length,
          data: locationData
        });
        if (onDataChangeRef.current) {
          onDataChangeRef.current(locationData);
        }
      }, 200);
    }
  }, [locationTree, propSelectedIds]);
  ...
  const getLocationData = useCallback((): SelectedLocation[] => {
    ...
    const countsByType = { COUNTRY: 0, STATE: 0, CITY: 0, ZIP: 0 };
    
    // First pass: count selections by type
    const countSelectedNodes = (nodes: LocationNode[]) => {
      nodes.forEach(node => {
        if (selectedIds.has(node.id)) {
          countsByType[node.type]++;
        }
        if (node.children && node.children.length > 0) {
          countSelectedNodes(node.children);
        }
      });
    };
    countSelectedNodes(locationTree);
    
    // Determine which geo level has selections (priority: ZIP > CITY > STATE > COUNTRY)
    const hasPostals = countsByType.ZIP > 0;
    const hasCities = countsByType.CITY > 0;
    const hasRegions = countsByType.STATE > 0;
    const hasCountries = countsByType.COUNTRY > 0;
    
    // Only process nodes that are actually selected AND match the most specific level
    const processSelectedNode = (node: LocationNode) => {
      if (!selectedIds.has(node.id)) return;
      
      // Only include nodes from the most specific level that has selections
      if (hasPostals && node.type !== 'ZIP') return;
      if (hasCities && node.type !== 'CITY') return;
      if (hasRegions && node.type !== 'STATE') return;
      if (hasCountries && node.type !== 'COUNTRY') return;
    ...
    console.log("📍 Most specific level:", hasPostals ? 'ZIP' : hasCities ? 'CITY' : hasRegions ? 'STATE' : hasCountries ? 'COUNTRY' : 'NONE');
