...
export default function Page() {
  ...
  const [selected, setSelected] = useState(() => {
    return (
      adFormats.find((f) => f.value === "NATIVE") ??
      adFormats.find((f) => f.value !== "DISPLAY") ??
      adFormats[0]
    );
  });
  ...
  const [disabledLocationIds, setDisabledLocationIds] = useState<Set<string>>(new Set()); // Locations explicitly disabled in API
  ...
  const handleLocationDataChange = useCallback(
    (
      ...
      // If we have disabled location IDs from API, select all EXCEPT those
      if (disabledLocationIds.size > 0 && locationData.length > 0) {
        console.log('📍 Selecting all locations except disabled ones...');
        const allLocationIds = locationData
          .filter(loc => loc.type === 'COUNTRY') // Only countries for now
          .map(loc => loc.id.toUpperCase());
        
        const selectedIds = new Set<string>(
          allLocationIds.filter(id => !disabledLocationIds.has(id))
        );
        
        console.log('📍 Total countries:', allLocationIds.length);
        console.log('📍 Disabled countries:', disabledLocationIds.size);
        console.log('📍 Selected countries (all except disabled):', selectedIds.size);
  ...
  const loadTargetingData = useCallback(async () => {
    try {
      ...
      // Step 2: Fetch targeting data from OfferNew/Ad API using the resolved offer ID
      const response = await axios.get(
        `https://panel.adsaro.com/admin/api/OfferNew/Ad/${offerId}?userToken=1wDtEkEz2ykyOdyx`
      );

      console.log("🎯 OfferNew/Ad targeting response:", response);
      if (response.status === 200) {
        ...
        // The API returns data directly, not wrapped in response.rows
        const targetingData = data;
        
        if (targetingData) {
          console.log("🎉 Processing targeting data:", targetingData);
          
          // Set location targeting - handle the format: Location.value = { "[id, TYPE]": {...} }
          if (targetingData.Location && targetingData.Location.value) {
            console.log(
              "📍 Loading location targeting from OfferNew/Ad:",
              targetingData.Location
            );
            
            const mapObj = targetingData.Location.value as Record<string, LocationEntry>;
            console.log(
              "📍 Location keys:",
              Object.keys(mapObj)
            );
            
            // Collect ALL location IDs that appear in API with enabled: false
            const disabledLocationIds = new Set<string>();
            
            Object.entries(mapObj).forEach(([key, loc]: [string, LocationEntry]) => {
              if (loc && typeof loc === "object") {
                const rawId = String(
                  (loc.id ?? key.replace(/^\[/, "").split(",")[0].trim())
                ).toUpperCase(); // Normalize to uppercase
                
                console.log(`📍 Processing location: ${rawId} (${loc.name}), enabled: ${loc.enabled}`);
                
                // Track locations that are explicitly disabled in API
                if (loc.enabled === false) {
                  disabledLocationIds.add(rawId);
                  console.log("❌ Location explicitly disabled in API:", rawId, loc.name);
                } else if (loc.enabled === true) {
                  console.log("✅ Location enabled in API:", rawId, loc.name);
                }
              }
            });
            
            // Store disabled IDs so Location component can exclude them
            setDisabledLocationIds(disabledLocationIds);
            console.log("📍 Disabled location IDs from API:", Array.from(disabledLocationIds));
            console.log("📍 Total disabled locations:", disabledLocationIds.size);
            
            // Don't set locationIds yet - wait for locationData to load, then select all except disabled
            setLocationIds(new Set());
            
            console.log("✅ Location targeting loaded - will select all except disabled locations");
          } else if (targetingData.Location && Array.isArray(targetingData.Location)) {
            // Fallback: handle array format
            console.log(
              "📍 Loading location targeting (array format):",
              targetingData.Location
            );
            const locationIdsSet = new Set(
              (targetingData.Location as LocationEntry[]).map((loc) =>
                String(loc.id ?? "").toUpperCase()
              )
            );
            setLocationIds(locationIdsSet);
            // Don't set locationData here - let EditLocation component build it
            // const locationData = (
            ...
            // setLocationData(locationData);
            console.log(
              "✅ Location targeting loaded (array):",
              locationIdsSet.size,
              "locations - waiting for EditLocation to build locationData"
            );
          } else {
            console.log(
    ...
    try {
      ...
      if (campaignData) {
        ...
        try {
          ...
          const adv = cd?.ad_vertical as unknown;
          if (adv != null && String(adv).length > 0) {
            setValue("ad_vertical", String(adv));
          }
        } catch {}
  ...
  const onSubmit = async (data: FormData) => {
    ...
    // Build locationEdit with ONLY selected locations (REPLACE mode will remove others)
    const selectedLocations = locationData.filter((loc) =>
      loc.id && loc.type && selectedLocationSet.has(loc.id)
    );
    
    const locationEdit = selectedLocations
      .reduce((acc, loc) => {
        const key = `[${loc.id.toLowerCase()}, ${loc.type}]`;
        acc[key] = {
          id: loc.id.toLowerCase(),
          type: loc.type,
          enabled: true, // ALL sent locations are enabled
          bid_adjustment: typeof loc.bid_adjustment === "number" ? loc.bid_adjustment : 1.0,
          name: loc.name,
          parent: null,
        };
        return acc;
    ...
    console.log('📍 EDIT - Location payload being sent:', {
      mode: "REPLACE", // Use REPLACE to remove old locations and only keep selected ones
      locationCount: Object.keys(locationEdit).length,
      locations: Object.keys(locationEdit)
    });
    ...
    try {
        ...
        if (!auth?.token) {
          ...
          if (!matchedOffer?.id && !resolvedOfferId) {
            ...
            type OfferUpdateCollectData = {
              ...
              Location?: {
                mode: "UPDATE" | "REPLACE";
                edit: Record<string, unknown>; // Object with keys like "[us, COUNTRY]"
              };
              OpsysNew?: {
                mode: "UPDATE";
            ...
            if (Object.keys(locationEdit).length > 0) {
              collectdata.Location = { mode: "REPLACE", edit: locationEdit }; // REPLACE to remove old locations
            }
            if (osEdit.length > 0) {
      ...
      try {
        if (campaignOk) {
          try {
            toast.success(
              "Campaign successfully updated and targeting applied",
              {
                position: "top-right",
                autoClose: 3000,
              }
      ...
      if (!campaignOk) {
        ...
        try {
          ...
          console.warn("Could not show error toast notification:", toastErr);
        }
      } else {
      }
    } catch (err) {
      ...
      try {
        ...
        console.warn("Could not show submission error toast:", toastErr);
      }
    } finally {
          ...
          <div className="@container/main gap-2">
            <div className="container sm:p-8 mx-auto py-4 md:gap-6 md:py-6">
              ...
              <nav className="mb-4 flex items-center space-x-2 text-sm text-gray-600">
                ...
                <span>/</span>
                <span className="text-gray-900 font-medium">Edit Campaign</span>
              </nav>
                ...
                {currentStep === 1 && (
                  <Card>
                    ...
                    <CardContent className="space-y-6">
                      {/* Ad Format Selection - Disabled for Edit */}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            {adFormats.map((format) => {
                              const active = selected.value === format.value;
                              return (
                                <button
                                  key={format.name}
                                  type="button"
                                  disabled
                                  className={`rounded-md border px-3 py-1.5 text-xs font-medium transition-colors cursor-not-allowed ${
                                    active
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          <Label htmlFor="ad_vertical">AD Vertical</Label>
                          <select
                            id="ad_vertical"
                        ...
                        <div>
                          ...
                          <select
                            ...
                            disabled
                            className="p-1.5 border block w-full rounded-md border-gray-300 shadow-sm bg-gray-100"
                          >
                            <option value="">
                        ...
                        {/* Date scheduling removed for edit - campaign dates managed elsewhere */}
{/* 
                        <div>
                ...
                {currentStep === 2 && (
                  <div className="space-y-6">
                    <Card>
                      ...
                      <CardContent className="space-y-6">
                        <LocationTargeting
                          key={`location-${locationIds.size}-${Array.from(locationIds).sort().join(',')}`}
                        ...
                        {/* Debug log */}
                        {(() => {
                          console.log('📍 EDIT PAGE - Passing to LocationTargeting:', {
