...
export const columns: ColumnDef<NotificationRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-md border ${cls}`}
        >
          {v}
        </span>
      );
...
export function DataTableDemoNo() {
  ...
  const handleView = (n: NotificationRow) => {
    ...
    if (!n.read) {
      ...
      setData((prev) =>
        prev.map((x) => (x.id === n.id ? { ...x, read: now } : x)),
      );
    }
  };
  ...
  const fetchData = async (start: string, end: string) => {
    ...
    // setLoading(true);
    try {
      let url = `https://panel.adsaro.com/publisher/api/Notifications/?version=4&token=${mytoken}`;
      if (start && end) {
