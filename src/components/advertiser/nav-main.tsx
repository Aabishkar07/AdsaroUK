...
type NavItem = {
  title: string
  url?: string // Make url optional for dropdown items
  icon?: LucideIcon
  items?: NavItem[] // 👈 support for subitems
}

export function AdvertiserNavMain({ items }: { items: NavItem[] }) {
        ...
        {open && (
          <div className="pl-3 ml-4 space-y-1 border-l">
            {item.items.map((subItem) => (
              <NavMenuItem key={subItem.title} item={subItem} pathname={pathname} />
            ))}
          </div>
        )}
      </div>
